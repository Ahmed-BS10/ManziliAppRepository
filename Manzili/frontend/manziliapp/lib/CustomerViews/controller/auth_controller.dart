...
import 'package:manziliapp/CustomerViews/model/login_model.dart';
import 'package:manziliapp/CustomerViews/model/store_create_model.dart';
import 'package:manziliapp/CustomerViews/model/user_create_model.dart';
...
class AuthController extends GetxController {
  ...
  final String _userRegisterEndpoint = "http://man.runasp.net/api/Auhencation/RegsiterUser";
  final String _storeRegisterEndpoint = "http://man.runasp.net/api/Auhencation/RegsiterStore";
  ...
  Future<void> registerUser(UserCreateModel user) async {
    isLoading.value = true;
    try {
      var request = http.MultipartRequest('POST', Uri.parse(_userRegisterEndpoint));
  ...
  Future<void> _buildUserRequest(http.MultipartRequest request, UserCreateModel user) async {
    request.fields.addAll(user.toMap().map((key, value) => MapEntry(key, value.toString())));
  ...
  Future<void> _buildStoreRequest(http.MultipartRequest request, StoreCreateModel storeData) async {
    ...
    if (storeData.image != null) {
      request.files.add(await http.MultipartFile.fromPath('Image', storeData.image!.path));
    }
  }
