...
namespace Manzili.Core.Services
{
    public class StoreServices
    {
        ...
        public async Task<OperationResult> CreateAsync(StoreCreateDto storeDto)
        {


            if (await _storeRepository.ExistsAsync(x => x.BusinessName == storeDto.BusinessName))
                return OperationResult.Failure("BusinessName already exists");
        ...
        public async Task<OperationResult> UpdateAsync(StoreUpdateDto newStore)
        {
            var oldStore = await _storeRepository.Find(x => x.Id == newStore.storeId);
            if (oldStore == null) return OperationResult.Failure("Store not found");
        ...
        {
            var store = await _storeRepository.Find(x => x.Id == id);
            if (store == null) return OperationResult.Failure("Store not found");
